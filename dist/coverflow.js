...
function initialize(imagesInit, container, options) {
  options = options || {};
  pressed = false;
  oldActiveIndex = 0;
  ...
  count = imagesInit.length;
  images = [];
  while (images.length < count) {
    var el;
    if (typeof imagesInit[images.length] === 'string') {
      el = document.getElementById(imagesInit[images.length]);
    } else {
      el = imagesInit[images.length];
    }
